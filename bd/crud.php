...
class Crud extends Conexion{
    ...
    public function consultarTodo(){
        try{
            ...
            $stm->execute();
            return $stm->fetchAll(PDO::FETCH_OBJ); //Se obtiene tipo objeto
    ...
    public function consultarUno(int $id){
        try{
            ...
            $stm->execute([$id]);
            return $stm->fetch(PDO::FETCH_OBJ); //Se obtiene tipo objeto
        }catch(PDOException $mensaje){
            echo $mensaje->getMessage();    
        }
    }

    public function eliminarUno(int $id){
        try{
    ...
    public function crear(string $columnas, string $marcadores, array $datos){
     ...
     public function modificacion(string $columnas, array $datos, int $id){
        $stm = $this->pdo->prepare("UPDATE $this->table SET $columnas WHERE id=$id");
        $stm->execute([$datos]);
    }
